...
class MainForm extends Canvas {
	...
	public class FormListGrid extends ListGrid {
		...
		// public native Boolean startEditingInt(int rowNum, int colNum, Boolean
		// suppressFocus) /*-{
		...
		//
		// @Override
		// public Boolean startEditing(Integer rowNum, Integer colNum, Boolean
		// suppressFocus) {
		// Utils.debug("FormListGrid.startEditing: " +
		// this.getJsObj().toString());
		// // return super.startEditing(rowNum, colNum, suppressFocus);
		// // 20121210 - косяк редактирования -
		// com.google.gwt.dev.shell.HostedModeException: invoke arguments: JS
		// value of type Java
		// // Object com.google.gwt.dev.shell.JsValueOOPHM$DispatchObjectOOPHM,
		// expected int
		...
		/*-{
			...
			var rowsJS = rows == null ? null
					: @com.smartgwt.client.util.JSOHelper::convertToJavaScriptArray([I)(rows);
			return self.saveAllEdits(rowsJS);
		}-*/;

		// Пример добавления хендлера для эвентов. См.
		// http://forums.smartclient.com/showthread.php?t=9923
		// @Override
		// public HandlerRegistration
		// addRowEditorEnterHandler(RowEditorEnterHandler handler) {
		// Utils.debug("FormListGrid.Adding handler");
		// HandlerRegistration r = doAddHandler(handler,
		// RowEditorEnterEvent.getType());
		// return r;
		// };
		...
		FormTreeGrid() {
			...
			this.addNodeClickHandler(new NodeClickHandler() {
				...
				public void onNodeClick(NodeClickEvent event) {
					...
					String titlePath = "-";
					// Error Something other than a Java object was returned
					// from JSNI method
					// '@com.smartgwt.client.widgets.tree.Tree::getTitle(Lcom/smartgwt/client/widgets/tree/TreeNode;)':JS
					// value of type int,
			...
			this.addFolderOpenedHandler(new FolderOpenedHandler() {
				...
				public void onFolderOpened(FolderOpenedEvent event) {
					Utils.debug("onFolderOpened>>>>"
							+ event.getNode().getTitle());
			...
			this.addFolderDropHandler(new FolderDropHandler() {
				...
				public void onFolderDrop(FolderDropEvent event) {
					...
					ActionItem dragAndDropActItem = mainFormPane
							.getButtonsToolBar().getDragAndDropActItem();
					FormActionMD faMD = (null != dragAndDropActItem) ? dragAndDropActItem
							.getFormActionMD() : new FormActionMD();
	...
	class GridRecordClickHandler implements RecordClickHandler {
		...
		public void onRecordClick(RecordClickEvent event) {
			...
			Utils.debug("MainForm.GridRecordClickHandler.onRecordClick.1. r:"
					+ r);
			if (null == r) {
				r = ((ListGrid) event.getSource()).getEditedRecord(event
						.getRecordNum());
				Utils.debug("MainForm.GridRecordClickHandler.onRecordClick.2. r:"
						+ r);
			}
	...
	class GridRowContextClickHandler implements RowContextClickHandler {
		...
		public void onRowContextClick(RowContextClickEvent event) {
			...
			// mainFormPane.setThisFormCriteria(r);
			mainFormPane.filterDetailData((ListGridRecord) r, treeGrid,
					event.getRowNum(), false, false, false);
		}
	}
	...
	public int getSelectedRecord() {
		...
		if (-999 == currRow) {
			try {
				...
				Utils.debug("MainForm.getSelectedRecord error:"
						+ e.getMessage() + "; currRow:" + currRow);
			}
		}
		return currRow;
	}
	...
	MainForm(final MainFormPane mainFormPane, final boolean showResizeBar,
		...
		if ("T".equals(formMetadata.getFormType())) {
			...
			// 20130604 - Для ListGrid добавлена возможность управления размером
			// страницы. Для дерева не нужно - само умное
		...
		if (formMetadata.isShowBottomToolBar()) {
			treeGrid.setGridComponents(new Object[] { ListGridComponent.HEADER,
		...
		{// Редактирование в гриде.
			...
			if (null != mainFormPane.getFormMetadata()
					.getDoubleClickActionCode()) {
				...
				treeGrid.addRecordDoubleClickHandler(new RecordDoubleClickHandler() {
					...
					public void onRecordDoubleClick(RecordDoubleClickEvent event) {
						try {
							mainFormPane.getButtonsToolBar()
									.getDoubleClickActItem()
									.doActionWithConfirm(event.getRecordNum());
						} catch (Exception e) {
							e.printStackTrace();
							Window.alert("treeGrid.addRecordDoubleClickHandler: "
									+ e.getMessage());
						}
					}
				});
		...
		// Utils.debugAlert("treeGrid.InitialSort:" +
		// treeGrid.getInitialSort());
		{
		...
		treeGrid.addEditorExitHandler(new EditorExitHandler() {
			...
			public void onEditorExit(EditorExitEvent event) {
				...
				if (null == event.getRecord()
						&& EditCompletionEvent.ESCAPE_KEYPRESS.equals(event
								.getEditCompletionEvent())) {
					int nextRecord = (0 == rowNum && null != grid
							.getRecord(rowNum + 1)) ? rowNum + 1 : rowNum - 1;
					System.out.println("rowNum:" + rowNum + "; nextRecord:"
							+ nextRecord);
					...
					mainFormPane.filterDetailData(grid.getRecord(nextRecord),
							treeGrid, nextRecord);
				}
		...
		// ListGrid grid = ((ListGrid) event.getSource());
		// int rowNum = event.getRowNum();
		// System.out.println("treeGrid.onRowEditorExit getEditCompletionEvent:"
		// + event.getEditCompletionEvent());
		// // Обработка выхода по клавише ESCAPE при отказе от ввода новой
		// записи
		// if (null == event.getRecord() &&
		// EditCompletionEvent.ESCAPE.equals(event.getEditCompletionEvent())) {
		// int nextRecord = (0 == rowNum && null != grid.getRecord(rowNum + 1))
		// ? rowNum + 1 : rowNum - 1;
		// System.out.println("rowNum:" + rowNum + "; nextRecord:" +
		// nextRecord);
		// // mainFormPane.getValuesManager().clearValues();
		...
		// grid.selectRecord(nextRecord);
		// mainFormPane.setSelectedRow(nextRecord);
		// mainFormPane.filterDetailData(grid.getRecord(nextRecord), treeGrid,
		// nextRecord);
		// }
		...
		treeGrid.addRowEditorEnterHandler(new RowEditorEnterHandler() {
			...
			@Override
			public void onRowEditorEnter(RowEditorEnterEvent event) {
				Utils.debug("treeGrid.onRowEditorEnter started... event:"
						+ event);
				ListGrid grid = ((ListGrid) event.getSource());
				...
				try {
					...
					// Utils.debugAlert("treeGrid.onRowEditorEnter. rowNum:" +
					// rowNum);
				} catch (Exception e) {
				...
				if (null == record && 0 == newRec.getAttributes().length) {
					...
					// TODO Простановка DisplayValue для лукап-форм с
					// DisplayFileds
					FormDataSourceField[] dsf = mainFormPane.getFormColumns()
							.getDataSourceFields();
					for (FormDataSourceField f : dsf) {
						Utils.debug("treeGrid.onRowEditorEnter."
								+ f.getColumnMD().getName() + ">>:"
								+ f.getColumnMD().getLookupDisplayValue());
					}
					Utils.debug("treeGrid.onRowEditorEnter.newRec.getAttributes():"
							+ newRec.getAttributes());
				}
	...
	// 20130514 - Для выделения цветом выбранной записи. Не взлетело. Вызов
	// закоменчен вверху.
	// TODO - вывести в настройки формы для возможности переопределения
	// (BackgroundColor)
	...
	public void setNewRecDefaultValues(final int rowNum,
			boolean isFromDefaultVals) {
		FormActionMD actMD = mainFormPane.getCurrentAction();
		final FormDataSourceField[] dsFields = mainFormPane.getFormColumns()
				.getDataSourceFields();
		if (isFromDefaultVals) {
			Map<String, Object> rowMap = Utils
					.getRowDefaultValuesMap(mainFormPane);
		...
		if (null == actMD.getDmlProcText()) {
			...
			DMLProcExecution procExec = new DMLProcExecution(mainFormPane) {
				...
				public void executeSuccessSubProc() {
					...
					Map<String, Object> resultMap = Utils.getMapFromRow(
							dsFields, getResultRow());
			...
			Row oldRow = null;
			Row newRow = Utils.getRowFromRecord(dsFields,
					treeGrid.getEditedRecord(rowNum));
	...
	public void doBeforeClose() {
		...
		Utils.debug(formCode + ": mainForm.doBeforeClose(). sessionId = "
				+ ConstructorApp.sessionId + "; gridHashCode = " + gridHashCode);
		FormMD formState = mainFormPane.getFormState();
		Utils.debug("mainFormPane.FormState: " + formState);
		Utils.createQueryService("MainForm.closeForm").closeForm(
				mainFormPane.getInstanceIdentifier(), formState,
				new DSAsyncCallback<Void>() {
					@Override
					public void onSuccess(Void result) {
						Utils.debug("MainForm. Form " + formCode + "("
								+ gridHashCode + ")" + " closed...");
					}
				});
	}
	...
	class MainFormResizedHandler implements ResizedHandler {
		...
		public void onResized(ResizedEvent event) {
			...
			try {
				...
				System.err
						.println("MainForm.MainFormResizedHandler.onResized(ResizedEvent event) error:");
				e.printStackTrace();
			}
			// Horizontal
			if (0 != hCnt) {
				Utils.debug("New Width for "
						+ mainFormPane.getFormCode()
						+ ": "
						+ (Math.round(mf.getWidth().floatValue()
								/ pts.getWidth().floatValue() * 100)));
			}
			// Vertical
			if (0 != vCnt) {
				Utils.debug("New Height for "
						+ mainFormPane.getFormCode()
						+ ": "
						+ (Math.round(mf.getHeight().floatValue()
								/ pts.getHeight().floatValue() * 100)));
			}
		}
	...
	public ExportData createExportData() {
		...
		title = "".equals(title) ? mainFormPane.getFormMetadata().getFormCode()
				: title;
		exportData.setTitle(title);

		for (FormTreeGridField f : mainFormPane.getFormColumns()
				.getGridFields()) {
			if ((!"false".equals(f.getAttribute("showIf")) && !"false".equals(f
					.getAttribute("canExport")))
					|| null != f.getColumnMD().getEditorTabCode()) {
				exportData.getHeaderNames().add(f.getName());
				exportData.getHeaderTitles().add(f.getTitle());
				String displayField = (f instanceof FormTreeGridField) ? f
						.getColumnMD().getLookupDisplayValue() : null;
				exportData.getHeaderDisplFldNames().add(displayField);

				String hdrType = "String";
				if ("N".equals(f.getColumnMD().getDataType())
						&& null == f.getColumnMD().getLookupCode()) {
					hdrType = "Number";
				}
	...
	public void exportData() {
		...
		try {
			...
			if (null != pfp) {
				...
				Record r = pfp.getMainForm().getTreeGrid()
						.getRecord(pfp.getSelectedRow());
				ArrayList<String> row = Utils.createArrayListFromRecord(r, pfp,
						parentRowData.getHeaderNames(),
						parentRowData.getHeaderDisplFldNames());
		...
		try {
			for (Record r : rs.getRange(0, dataLen)) {
				ArrayList<String> row = Utils.createArrayListFromRecord(r,
						mainFormPane, exportData.getHeaderNames(),
						exportData.getHeaderDisplFldNames());
		...
		Utils.createQueryService("MainForm.setExportData").setExportData(
				mainFormPane.getInstanceIdentifier(), exportData,
	...
	public void exportGrid(final String url) {
		...
		// TODO - Проблема в Firefox - для больших диапазонов (данных более 32К)
		// падает с ошибкой
