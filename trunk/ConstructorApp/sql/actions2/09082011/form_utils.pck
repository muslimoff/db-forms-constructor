...
CREATE OR REPLACE PACKAGE BODY form_utils As
   ...
   Begin
      ...
      --l_res    := EXT.XX_REG_EX.REPLACE_WITH (l_res, chr(38) || 'fc_schema_owner.', applications_pkg.get_fc_schema);
      --l_res    := EXT.XX_REG_EX.REPLACE_WITH (l_res, chr(38) || 'db_username.', User);
      l_res    := REPLACE (l_res, '&' || 'fc_schema_owner.', applications_pkg.get_fc_schema);
      l_res    := REPLACE(l_res, '&' || 'db_username.', User);
...
/*   Function describe_columns (p_sql_text Varchar2, p_form_code Varchar2 Default Null)
   ...
   Function convert_to_desc_t_idx (dsc DBMS_SQL.desc_tab, p_form_code Varchar2 Default Null)
      ...
      l_result   desc_idx_t;
   Begin
      If dsc.count > 0 Then -- dos10062011
...
/*  And Exists (Select 1
   ...
   Begin
      Begin
         ...
         End Loop;
         If src.count > 0 Then -- dos10062011
           For i In src.First .. src.Last Loop
...
--            Raise;
   ...
   Begin
      ...
      exception when others then l_res.pkg_txt := 'Error in form_utils:' ||sqlerrm;
      return l_res;
                      ...
                      And Not Exists (
                             Select Null
                              ...
                              Where t.entity_type In ('EDIT_FRM', 'TAB')
                               ...
                               And t.master_form_code = p_master_form_code
