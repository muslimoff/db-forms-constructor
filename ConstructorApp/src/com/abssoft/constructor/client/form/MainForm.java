...
class MainForm extends Canvas {
	...
	public class FormListGrid extends ListGrid {
		...
		// // 20121210 - косяк редактирования - com.google.gwt.dev.shell.HostedModeException: invoke arguments: JS value of type Java Object com.google.gwt.dev.shell.JsValueOOPHM$DispatchObjectOOPHM, expected int
		...
		FormTreeGrid() {
			...
			this.addNodeClickHandler(new NodeClickHandler() {
				...
				public void onNodeClick(NodeClickEvent event) {
					...
					String titlePath = "-";
					// Error Something other than a Java object was returned from JSNI method '@com.smartgwt.client.widgets.tree.Tree::getTitle(Lcom/smartgwt/client/widgets/tree/TreeNode;)':JS value of type int, expected java.lang.Object
					try {
			...
			this.addFolderDropHandler(new FolderDropHandler() {
				...
				public void onFolderDrop(FolderDropEvent event) {
					// Очищаем код действия формы в случае перетаскивания грида. Прийдется тут ловить действие, которое привязать к форме
	...
	MainForm(final MainFormPane mainFormPane, final boolean showResizeBar, HeaderSpan[] headerSpans) {
		...
		if (formMetadata.isShowBottomToolBar()) {
			treeGrid.setGridComponents(
					new Object[] { ListGridComponent.HEADER, ListGridComponent.FILTER_EDITOR, ListGridComponent.BODY, bottomToolBar }); // ListGridComponent.SUMMARY_ROW
		}
		// 20110322
			...
			public void onRowEditorEnter(RowEditorEnterEvent event) {
				...
				if (null == record && 0 == newRec.getAttributes().length) {
					...
					for (FormDataSourceField f : dsf) {
						Utils.debug(
								"treeGrid.onRowEditorEnter." + f.getColumnMD().getName() + ">>:" + f.getColumnMD().getLookupDisplayValue());
					}
		...
		treeGrid.setDataProperties(new ResultSet() {
			{
				this.setUseClientSorting(false);
				this.setUseClientFiltering(false);
				this.setNeverDropUpdatedRows(true);
				//this.setUpdatePartialCache(true);
				//this.setCriteriaPolicy(CriteriaPolicy.DROPONSHORTENING);
			}
		});
	}
	...
	public void exportData() {
		...
		try {
			...
			if (null != pfp) {
				...
				ArrayList<String> row = Utils.createArrayListFromRecord(r, pfp, parentRowData.getHeaderNames(),
		...
		try {
			for (Record r : rs.getRange(0, dataLen)) {
				ArrayList<String> row = Utils.createArrayListFromRecord(r, mainFormPane, exportData.getHeaderNames(),
