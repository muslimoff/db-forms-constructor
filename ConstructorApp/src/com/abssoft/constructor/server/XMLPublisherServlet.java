...
public class XMLPublisherServlet extends HttpServlet {
		...
		if (null == session) {
			...
			while (e.hasMoreElements()) {
				...
				if ("dataClobId".equals(paramName) && !"".equals(paramValue)) {
					try {
						dataClobId = Integer.valueOf(paramValue);
						System.out.println("dataClobId:" + dataClobId);
					} catch (Exception ee) {
					}
				}
			...
			if ("xmlp".equals(type)) {
				processXMLP(req, resp, session, contentDisposition, ContentType, template, dataClobId, filename);
				return;
			}
			if ("clob".equals(type)) {
				getCLOB(req, resp, session, contentDisposition, ContentType, dataClobId);
				return;
			}
		}
	}
		...
		while (formsIt.hasNext()) {
			...
			while (formsInstIt.hasNext()) {
				...
				if (frmInst.getClobHM().containsKey(dataClobId)) {
					CLOB result = frmInst.getClobHM().get(dataClobId);
	...
	public void processXMLP(HttpServletRequest req, HttpServletResponse resp, Session session, String contentDisposition,
		...
		try {
			...
			CLOB xmlDataClob = findCLOB(session, dataClobId);
			/** RTF to XSL FO **/
