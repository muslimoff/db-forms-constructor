...
-- Generated 19.10.2013 8:49:42 from FC22@KZM_TEST

-- Drop the old instance of MENUS_PKG
Drop Package fc22.menus_pkg
/
...
PACKAGE fc22.menus_pkg As
  ...
  Function p_get_menu_tree(p_menu_code    Varchar2,
    ...
    Return menu_tree_table
    Pipelined;
  function get_menu_code(p_menu_name in varchar2, p_form_code varchar2)
    return varchar2;
  -- procedure generate_by_path(p_path in varchar2, p_form_code varchar2);

End;
/

-- Grants for Package
Grant Execute On fc22.menus_pkg To fc22_admin
/
Grant Execute On fc22.menus_pkg To apps
/
...
PACKAGE BODY fc22.menus_pkg As
  ...
  */
  Function p_get_menu_tree(p_menu_code    Varchar2,
    ...
    Function append_to_collection(p_complete_collection menu_tree_table,
                                  p_part                menu_tree_table)
    ...
    Begin
      ...
      Else
        Null;
        l_complete_collection := p_complete_collection multiset union all
                                 p_part;
      End If;
    ...
    Begin
      l_rec := append_to_collection(p_complete_collection => l_rec,
                                    p_part                => l_rec_tmp);
    End;
  Begin
    ...
    If (p_element_type = 'FORM') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Настройки формы</i>' menu_name,
               p_menu_code parent_menu_code,
               Null As menu_form_code,
               'FORMS' form_display_stat,
               1 menu_position,
               14 icon_id,
               'Y' is_folder,
               l_element_type element_type,
               'Y' show_in_navigator,
               'N' As can_accept_drop,
               'N' As can_drag,
               p_form_display As form_code,
               Null || '' As tab_code,
               p_column_code As column_code,
               p_action_code As p_action_code,
               p_lookup_code As lookup_code Bulk Collect
          Into l_rec_tmp
          From DUAL;
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>В работе: Depends On</i>' menu_name,
               p_menu_code parent_menu_code,
               '' menu_form_code,
               '' form_display_stat,
               2 menu_position,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>В работе: Dependent Objects</i>' menu_name,
               p_menu_code parent_menu_code,
               '' menu_form_code,
               '' form_display_stat,
               3 menu_position,
    ...
    Elsif (p_element_type = 'DATA') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Колонки [<b>' || Count(*) || '</b>]</i>' As menu_name,
               p_menu_code parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMNS' form_display_stat,
               1 menu_position,
               11 icon_id,
               DECODE(Count(*), 0, 'N', 'Y') is_folder,
               l_element_type element_type,
               'Y' show_in_navigator,
               'N' As can_accept_drop,
               'N' As can_drag,
               p_form_code As form_code,
               p_tab_code As tab_code,
               p_column_code As column_code,
               p_action_code As p_action_code,
               p_lookup_code As lookup_code Bulk Collect
          Into l_rec_tmp
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Действия [<b>' || Count(*) || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_ACTIONS' As form_display_stat,
               2 As menu_position,
               13 As icon_id,
               DECODE(Count(*), 0, 'N', 'Y') As is_folder,
               l_element_type As element_type,
               'Y' As show_in_navigator,
               'N' As can_accept_drop,
               'N' As can_drag,
               p_form_code As form_code,
               p_tab_code As tab_code,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Вкладки [<b>' || Count(*) || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_TABS' As form_display_stat,
               3 As menu_position,
               12 As icon_id,
               DECODE(Count(*), 0, 'N', 'Y') As is_folder,
               l_element_type As element_type,
    ...
    Elsif (p_element_type In ('COLUMNS', 'ACTION_COLUMNS', 'TAB_COLUMNS')) Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs ||
               c.column_code As menu_code,
               DECODE(c.exists_in_metadata_flag,
                      'Y',
                      c.column_user_name || ' [<b>' || c.column_code ||
                      '</b>]',
                      ' <font color="red">[<b>' || c.column_code ||
                      '</b>]</font>') As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMNS' As form_display_stat,
               c.column_display_number As menu_position,
               11 icon_id,
               Case
               -- 1. Действия поля
                 When 0 != (Select Count(*)
                             ...
                             Where fca.form_code = c.form_code
                               And fca.column_code = c.column_code) Then
                  'Y'
               --2. аттрибуты
                 When 0 != (Select Count(*)
                              From form_column_attr_vals fcav
                             Where fcav.form_code = c.form_code
                               And fcav.column_code = c.column_code) Then
                  'Y'
               -- 3. лукап простой
                 When c.lookup_code Is Not Null And c.field_type In (8, 10) Then
                  'Y'
               -- 4. лукап-форма
                 When c.lookup_code Is Not Null And c.field_type In (9) Then
                  'Y'
               -- 5. вкладка
                 When c.editor_tab_code Is Not Null Then
                  'Y'
                 Else
                  'N'
               End As is_folder,
          ...
          Into l_rec_tmp
          From Table(form_utils.describe_form_columns_pl(p_form_code)) c
         Where ((p_element_type = 'ACTION_COLUMNS' And Exists
                (Select 1
                    From form_column_actions fca
                   Where fca.form_code = c.form_code
                     And fca.column_code = c.column_code
                     And fca.action_code = p_action_code)) Or
               (p_element_type = 'TAB_COLUMNS' And
               c.editor_tab_code = p_tab_code) Or
               p_element_type Not In ('ACTION_COLUMNS', 'TAB_COLUMNS'));
    ...
    Elsif (p_element_type = 'COLUMN') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>ColumnActions [<b>' || Count(Distinct c.action_code) ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMN_ACTIONS' As form_display_stat,
               1 As menu_position,
               14 As icon_id,
               'Y' As is_folder,
               l_element_type As element_type,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>ColumnAttributes [<b>' ||
               Count(Distinct c.attribute_code) || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMN_ATTR_VALS' As form_display_stat,
               1 As menu_position,
               14 As icon_id,
               'Y' As is_folder,
               l_element_type As element_type,
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs ||
               c.lookup_code As menu_code,
               '<i>StaticLookup: [<b>' || c.lookup_code || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'LOOKUPS' As form_display_stat,
               1 As menu_position,
               14 As icon_id,
               'N' As is_folder,
               l_element_type As element_type,
               ...
               p_column_code As column_code,
               p_action_code As p_action_code,
               c.lookup_code As lookup_code Bulk Collect
          Into l_rec_tmp
          From form_columns c
         Where c.form_code = p_form_code
           And c.column_code = p_column_code
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs || c.form_code As menu_code,
               '<i>FormLookup: ' || f.form_name || '[<b>' || f.form_code ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code --
              ,
               c.lookup_code As menu_form_code,
               Null As form_display_stat,
               1 As menu_position,
               NVL(f.icon_id, Null) As icon_id,
               'Y' As is_folder,
               l_element_type As element_type,
               'Y' As show_in_navigator,
               'N' As can_accept_drop,
               'N' As can_drag,
               f.form_code As form_code,
               '' As tab_code,
               '' As column_code,
               '' As p_action_code,
               '' As lookup_code Bulk Collect
          Into l_rec_tmp
          From form_columns c, forms f
         Where c.form_code = p_form_code
           And c.column_code = p_column_code
           And c.lookup_code = f.form_code
           And c.field_type In (9);
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs || t.tab_code As menu_code,
               '<i>Tab: ' || t.tab_name || ' [<b>' || t.tab_code ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_TABS' As form_display_stat,
               t.tab_display_number As menu_position,
    ...
    Elsif (p_element_type = 'COLUMN_ATTRIBUTES') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs ||
               c.form_column_attr_val_id As menu_code,
               '<i>' || c.attribute_code || ' = <b>' || c.attribute_value ||
               '</b></i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               '' As form_display_stat,
               1 As menu_position,
               14 As icon_id,
               'N' As is_folder,
               l_element_type As element_type,
    ...
    Elsif (p_element_type In
          ('ACTIONS', 'COLUMN_ACTIONS', 'ACTION_CHILD_ACTIONS')) Then
      Begin
        --1.1.1.1.2.1. ACTION
        --1.1.1.1.3.1.1.1. ACTION
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs ||
               c.action_code As menu_code,
               c.action_display_name || ' [<b>' || c.action_code || '</b>]' As menu_name,
               p_menu_code parent_menu_code,
               Null As menu_form_code,
               'FORM_ACTIONS' As form_display_stat,
               c.display_number As menu_position,
               c.icon_id As icon_id,
               Case
                 When
                 --1.1.1.1.2.1.1. ACTION_PLSQL
                  procedure_name Is Not Null Then
                  'Y'
                 When
                 --1.1.1.1.2.1.2. ACTION_COLUMNS
                  0 != (Select Count(*)
                          From form_column_actions fca
                         Where fca.form_code = c.form_code
                           And fca.action_code = c.action_code) Then
                  'Y'
                 When
                 --1.1.1.1.2.1.3. ACTION_CHILD_FORM
                  child_form_code Is Not Null Then
                  'Y'
                 When
                 --1.1.1.1.2.1.4. ACTION_PARENT_ACTION
                  parent_action_code Is Not Null Then
                  'Y'
                 When
                 --1.1.1.1.2.1.5. ACTION_CHILD_ACTIONS
                  0 != (Select Count(*)
                         ...
                         Where c1.parent_action_code = c.action_code
                           And c1.form_code = c.form_code) Then
                  'Y'
                 Else
                  'N'
               End As is_folder,
               ...
               p_tab_code As tab_code,
               p_column_code As column_code,
               c.action_code As action_code,
               p_lookup_code As lookup_code Bulk Collect
          Into l_rec_tmp
          From form_actions c
         Where c.form_code = p_form_code
           And ((p_element_type = 'COLUMN_ACTIONS' And Exists
                (Select 1
                    From form_column_actions fca
                   Where fca.form_code = c.form_code
                     And fca.column_code = p_column_code
                     And fca.action_code = c.action_code)) Or
               (p_element_type = 'ACTION_CHILD_ACTIONS' And Exists
                (Select 1
                    From form_actions c1
                   Where c1.form_code = c.form_code
                     And c1.action_code = p_action_code
                     And c1.action_code = c.parent_action_code)) Or
               p_element_type Not In
               ('COLUMN_ACTIONS', 'ACTION_CHILD_ACTIONS'));
    ...
    Elsif (p_element_type = 'ACTION') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>В работе! PL/SQL [<b>' || c.procedure_name ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               '' As form_display_stat,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Колонки (триггеры) [<b>' ||
               Count(Distinct c.column_code) || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMN_ACTIONS' As form_display_stat,
               2 As menu_position,
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs || c.form_code As menu_code,
               '<i>ChildForm: ' || f.form_name || '[<b>' || f.form_code ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code --
              ,
               f.form_code As menu_form_code,
               Null As form_display_stat,
               3 As menu_position,
               NVL(f.icon_id, Null) As icon_id,
               'Y' As is_folder,
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs ||
               p.action_code As menu_code,
               '<i>ParentAction: ' || p.action_display_name || '[<b>' ||
               p.action_code || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code --
              ,
               '' As menu_form_code,
               'FORM_ACTIONS' As form_display_stat,
               3 As menu_position,
               NVL(p.icon_id, Null) As icon_id,
               'Y' As is_folder,
               l_element_type As element_type,
               'Y' As show_in_navigator,
               'N' As can_accept_drop,
               ...
               '' As tab_code,
               '' As column_code,
               p.action_code As p_action_code,
               '' As lookup_code Bulk Collect
          Into l_rec_tmp
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>ChildActions [<b>' || Count(Distinct c.action_code) ||
               '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               Null
    ...
    Elsif (p_element_type = 'TABS') Then
      Begin
        ...
        Select p_menu_code || l_ps || l_element_type || l_vs || c.tab_code As menu_code,
               c.tab_name || ' [<b>' || c.tab_code || '</b>]' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_TABS' As form_display_stat,
               c.tab_display_number As menu_position,
               c.icon_id As icon_id,
               'Y' is_folder,
               l_element_type As element_type,
               'Y' As show_in_navigator,
    ...
    Elsif (p_element_type = 'TAB') Then
      Begin
         ...
         (Select t.child_form_code As form_code
          ...
          Select t.child_form_code As form_code
           ...
           Where t.form_code = p_form_code
             And t.tab_code = p_tab_code)
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Дочерние формы [<b>' || Count(*) || '</b>]</i>' As menu_name,
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               '' As form_display_stat,
               1 As menu_position,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               ...
               p_menu_code As parent_menu_code,
               Null As menu_form_code,
               'FORM_TAB_PARENT_EXCLNS' As form_display_stat,
               2 As menu_position,
               14 As icon_id,
               DECODE(Count(*), 0, 'N', 'Y') As is_folder,
               l_element_type As element_type,
        ...
        Select p_menu_code || l_ps || l_element_type As menu_code,
               '<i>Зависимые столбцы [<b>' || Count(*) || '</b>]</i>' menu_name,
               p_menu_code parent_menu_code,
               Null As menu_form_code,
               'FORM_COLUMNS' form_display_stat,
               3 menu_position,
      ...
      Begin
        ...
        c As
         (Select f.form_code,
                 f.form_name,
                 f.icon_id,
                 ROW_NUMBER() Over(Order By f.form_code) As pos
            From t, forms f
           Where t.form_code = f.form_code)
        Select p_menu_code || l_ps || l_element_type || l_vs || c.form_code As menu_code,
               c.form_name || ' [<b>' || c.form_code || '</b>]' As menu_name,
               p_menu_code As parent_menu_code,
               c.form_code As menu_form_code,
               Null As form_display_stat,
               c.pos As menu_position,
               NVL(c.icon_id, Null) As icon_id,
               'Y' As is_folder,
               l_element_type As element_type,
               'Y' As show_in_navigator,
               'N' As can_accept_drop,
        ...
        Select m.menu_code,
               ...
               DECODE(m.menu_form_code,
                      ...
                      DECODE((Select Count(*)
                             ...
                             'Y'),
                      'Y') As is_folder,
               TO_CHAR(DECODE(m.menu_form_code, Null, 'FOLDER', 'FORM')) As element_type,
               m.show_in_navigator,
               DECODE(m.menu_form_code, Null, 'Y', 'N') As can_accept_drop,
               'Y' As can_drag,
               p_form_code As form_code,
               p_tab_code As tab_code,
               p_column_code As column_code,
    ...
    If l_rec Is Not Null And l_rec.Count != 0 Then
      Declare
      ...
      Begin
        For i In l_rec.First .. l_rec.Last Loop
          ...
          ll_entity_code := Replace(l_rec(i).menu_code,
                                    l_rec(i).parent_menu_code);

          If ll_entity_code Like '%' || l_vs || '%' And l_rec(i)
            .parent_menu_code Like '%' || ll_entity_code || '%' Then
            l_rec(i).is_folder := 'N';
  ...
  begin
    select m.menu_code
     ...
     where m.menu_name = p_menu_name
       and m.menu_form_code = p_form_code;
  end get_menu_code;

/* procedure generate_by_path(p_path in varchar2, p_form_code varchar2) is
  begin
    select  from menus m
    if()
    end if;
  end generate_by_path;
 */
End;
/
